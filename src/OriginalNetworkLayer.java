...
/**
 * Created by Sumi on 2016/10/15.
 */
public class OriginalNetworkLayer {
    ...
    public static void formationOfOpinion(){
        ...
        for(int i=0;i<shufflenumber.size();i++){
            ...
            if(network[ln].friendagent[an1].size() ==0)
                continue;
            an2number = Paramerter.rand.nextInt(network[ln].friendagent[an1].size());
            an2 = network[ln].friendagent[an1].get(an2number).number;
