...
public class DataOutputForOriginal {
    ...
    public static void outPut(double allowance){
        ...
        try {
            ...
            file5 = new File("D:\\4年\\研究室\\卒業論文\\研究出力\\411.csv");
            if ( !file5.exists())
    ...
    public static void opinionAspect(int layernumber,int loopnumber){
        if(loopnumber == looptime){
            try {
                for(int i=0;i<Paramerter.agentnumber;i++){
                    filewriter5.write(loopnumber + "," + layernumber+ "," + i + "," + OriginalNetworkLayer.network[layernumber].agent[i].opinion+"\n");
                }
            }catch (IOException e) {
                System.out.println(e);
            }
